...
void insert_sort(PNODE *pHead);      //插入排序
...
void insert_sort(PNODE *pHead)
{
    ...
    PNODE head = (*pHead)->pNext;  //首指针
    head->pNext = NULL;
    while(cur)
    {
        next = cur->pNext;  //暂存下一个节点
        ...
        { 
            //中间插入
            PNODE prev = head;        //用来比较的节点的前一个节点
            PNODE phead = prev->pNext; //比较的节点
            while(phead)
            {        
